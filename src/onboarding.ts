...
async function noteDingTalkHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "4. Choose message receiving mode:",
      "   - Stream mode: best for local/private deployments and full AI card support",
      "   - HTTP callback mode: best for reverse proxy / multi-instance deployments",
      "5. If you run multiple local HTTP listeners, each DingTalk account must use a distinct httpPort",
      "6. Copy Client ID (AppKey) and Client Secret (AppSecret)",
